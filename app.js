...
bars && list && bars.addEventListener('click', () =>{
...
// Update user icon link (login/profile) based on auth state
document.addEventListener('DOMContentLoaded', function() {
   try {
      const isLoggedIn = !!localStorage.getItem('token');
      const target = isLoggedIn ? '/profile.html' : '/login.html';
      const label = isLoggedIn ? 'الملف الشخصي' : 'تسجيل الدخول';
      document.querySelectorAll('a').forEach(a => {
         const userIcon = a.querySelector('i.fa-user, i.user, i#userIcon');
         if (userIcon) {
