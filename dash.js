...
if (!window.Courses) {
    window.Courses = {
        ...
        manageCourseContent: function(courseId, courseTitle) {
            ...
            .then(response => {
                if (!response.ok) throw new Error('فشل في جلب محتوى الكورس');
                return response.json();
            })
            .then(courseContent => {
                this.displayCourseContent(courseContent);
                modal.style.display = 'flex';
                modal.classList.add('show');
                document.body.style.overflow = 'hidden';
            })
        ...
        displayCourseContent: function(courseContent) {
...
(function ensureCanonicalLessonEdit(){
  ...
  async function canonicalHandleEditLessonSubmit(e){
    ...
    try{
      ...
      const res = await fetch(`/api/courses/unit/${window.currentUnitId}/lesson/${window.currentLessonId}` , {
        method: 'PUT',
...
async function handleAddLessonSubmit(e) {
    ...
    try {
        ...
        console.log('Refreshed course content after edit:', content);
...
function displayCourseContent(courseContent) {
    console.log('displayCourseContent called with:', courseContent);
    ...
    if (!courseContent.units || courseContent.units.length === 0) {
        container.innerHTML = '<p>لا توجد وحدات لهذا الكورس</p>';
        return;
    }
    ...
    if (!currentCourseId) {
        ...
        container.innerHTML = '<p>خطأ: لم يتم تحديد الكورس</p>';
        return;
    }
    
    courseContent.units.forEach((unit, unitIndex) => {
        const unitDiv = document.createElement('div');
        unitDiv.className = 'module-item';
        unitDiv.innerHTML = `
            <div class="module-header">
                <h4>الوحدة ${unitIndex + 1}: ${unit.title}</h4>
                <div class="module-actions">
                    <button onclick="addLessonToUnit('${currentCourseId}', '${unit._id}')" class="btn btn-sm btn-primary">
                        <i class="fas fa-plus"></i> إضافة درس
                    </button>
                    <button onclick="deleteUnit('${currentCourseId}', '${unit._id}')" class="btn btn-sm btn-danger">
            ...
            <div class="lessons-list">
                ${unit.lessons.map((lesson, lessonIndex) => `
                    <div class="lesson-item">
                        <span>الدرس ${lessonIndex + 1}: ${lesson.title}</span>
                        <div class="lesson-actions">
                            <button onclick="editLesson('${currentCourseId}', '${unit._id}', '${lesson._id}')" class="btn btn-sm btn-secondary">
                                <i class="fas fa-edit"></i>
                            </button>
                            <button onclick="deleteLesson('${currentCourseId}', '${unit._id}', '${lesson._id}')" class="btn btn-sm btn-danger">
                                <i class="fas fa-trash"></i>
                            </button>
                        </div>
                    </div>
...
function openAddUnitModal() {
    ...
    try {
        if (typeof ModalSystem !== 'undefined' && ModalSystem && typeof ModalSystem.openModal === 'function') {
            ModalSystem.openModal('addUnitModal');
...
async function handleAddUnitSubmit(e) {
    ...
    try {
        ...
        await addUnitToCourseAPI(currentState.courseId, unitData);
        alert('تم إضافة الوحدة بنجاح!');
        
        closeAddUnitModal();
        
        // إعادة تحميل محتوى الكورس
        manageCourseContent(currentState.courseId, currentState.courseTitle);
...
async function handleAddLessonSubmit(e) {
    ...
    try {
        ...
        alert('تم إضافة الدرس بنجاح!');
        closeAddLessonModal();
        manageCourseContent(window.currentState.courseId, window.currentState.courseTitle);
...
async function deleteUnit(courseId, unitId) {
    ...
    try {
        ...
        // إعادة تحميل محتوى الكورس
        manageCourseContent(courseId, currentState.courseTitle);
    } catch (error) {
...
async function deleteLesson(courseId, unitId, lessonId) {
    ...
    try {
        ...
        alert('تم حذف الدرس بنجاح!');
        // إعادة تحميل محتوى الكورس
        manageCourseContent(courseId, currentCourseTitle);
    } catch (error) {
