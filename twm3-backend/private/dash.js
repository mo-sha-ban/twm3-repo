...
function displayCourseContent(courseContent, course) {
    ...
    const isPaidRobust = (course && (
        (typeof course.isFree !== 'undefined' && course.isFree === false) ||
        (typeof course.isPaid !== 'undefined' && course.isPaid === true) ||
        (typeof course.price !== 'undefined' && Number(course.price) > 0) ||
        (typeof course.paid !== 'undefined' && course.paid === true)
    ));

    if (promoSection) {
        promoSection.style.display = isPaidRobust ? 'block' : 'none';
        // Also toggle the add-promo-button if present
        const addPromoBtn = document.getElementById('add-promo-button');
        if (addPromoBtn) addPromoBtn.style.display = isPaidRobust ? 'inline-block' : 'none';

        if (isPaidRobust) {
            // Load existing promo video URL if available
            const promoUrlInput = document.getElementById('promoVideoUrl');
            if (promoUrlInput && course && course.promoVideo) {
                promoUrlInput.value = course.promoVideo;
                // Update the iframe preview if there's a valid URL
    ...
    // Show/hide the 'Add Promo Video' button next to 'Add Unit'
    try {
        const addPromoBtn = document.getElementById('add-promo-button');
        if (addPromoBtn) addPromoBtn.style.display = isPaid ? 'inline-block' : 'none';
    } catch (e) {
        console.warn('Could not toggle add-promo-button visibility', e);
    }
...
// Open / focus the promo editor inside the course content modal
function openPromoEditor(courseId) {
    try {
        const promoSection = document.getElementById('promoVideoSection');
        if (!promoSection) return;
        // Ensure promo section is visible (it should be for paid courses)
        promoSection.style.display = 'block';
        // If modal is hidden for some reason, open it
        const modal = document.getElementById('courseContentModal');
        ...
        // Scroll promo section into view and focus URL input
        setTimeout(() => {
            promoSection.scrollIntoView({ behavior: 'smooth', block: 'start' });
            const promoUrl = document.getElementById('promoVideoUrl');
            if (promoUrl) promoUrl.focus();
        }, 120);
    } catch (err) {
        console.error('openPromoEditor failed', err);
    }
}

window.openPromoEditor = openPromoEditor;

// Select promo type: 'link' or 'upload'
function selectPromoType(type) {
    try {
        const linkGroup = document.getElementById('promoLinkGroup');
        const uploadGroup = document.getElementById('promoUploadGroup');
        if (type === 'upload') {
        ...
        console.warn('selectPromoType failed', err);
    }
}
